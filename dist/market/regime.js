...
const log = (0, logger_1.createComponentLogger)('market:regime');
class MarketRegimeDetector {
    detectRegime(candles, spyCandles) {
        ...
        const atrValue = (0, indicators_1.atr)(candles, 14);
        const macdSignal = this.calculateMACDSignal(candles);
        const volumeExpansion = this.calculateVolumeExpansion(candles);
        const ema21 = (0, indicators_1.ema)(candles.map((c) => c.close), 21);
        const ema50 = (0, indicators_1.ema)(candles.map((c) => c.close), 50);
        let trend = 'neutral';
    ...
    calculateVolumeExpansion(candles) {
        const recentLength = 5;
        if (candles.length < recentLength + 5)
            return 1;
    ...
    calculateMACDSignal(candles) {
        if (candles.length < 26)
            return 'neutral';
        const ema12 = (0, indicators_1.ema)(candles.map((c) => c.close), 12);
        const ema26 = (0, indicators_1.ema)(candles.map((c) => c.close), 26);
