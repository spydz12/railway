...
exports.getPerformanceValidationAnalytics = getPerformanceValidationAnalytics;
exports.getSignalQualityValidationAnalytics = getSignalQualityValidationAnalytics;
exports.getPaperTradingAnalytics = getPaperTradingAnalytics;
const queries_1 = require("../database/queries");
function toNum(value) {
    return typeof value === 'number' ? value : 0;
}
function calcRollingWinRate(outcomes, window = 20) {
    const result = [];
...
async function getPerformanceValidationAnalytics() {
    const performances = await (0, queries_1.getAllSignalPerformances)(5000);
    const byStrategy = new Map();
    for (const row of performances.slice().reverse()) {
        if (!byStrategy.has(row.strategy))
            byStrategy.set(row.strategy, []);
        byStrategy.get(row.strategy).push(row);
    }
    const leaderboard = Array.from(byStrategy.entries()).map(([strategy, rows]) => {
...
async function getSignalQualityValidationAnalytics() {
    const performances = await (0, queries_1.getAllSignalPerformances)(5000);
...
async function getPaperTradingAnalytics() {
    ...
    const [positions, snapshots] = await Promise.all([
        (0, queries_1.getRecentPaperPositions)(500),
        (0, queries_1.getPaperEquitySnapshots)(account.id, 1500),
    ]);
