...
exports.formatNewIdeaMessage = formatNewIdeaMessage;
exports.formatUpdateMessage = formatUpdateMessage;
const indicators_1 = require("../utils/indicators");
const config_1 = require("../config");
const logger_1 = require("../utils/logger");
const log = (0, logger_1.createComponentLogger)('telegram:formatter');
...
function formatNewIdeaMessage(idea) {
    ...
    const filteredReasons = Array.isArray(idea.reasons)
        ...
        : [];
    const fallbackReason = idea.reason && !EXCLUDED_KEYWORDS.test(idea.reason) ? idea.reason : null;
    const reasonsText = filteredReasons.length > 0
    ...
    const invalidationBlock = idea.invalidation_rule && idea.invalidation_rule.trim()
    ...
    const portfolioRiskLabel = idea.portfolioRiskLabel ? esc(idea.portfolioRiskLabel) : null;
    const riskGuardState = idea.riskGuardState ? esc(idea.riskGuardState) : null;
    const sessionContext = idea.sessionContext ? idea.sessionContext : null;
    const marketStressLevel = idea.marketStressLevel ? esc(idea.marketStressLevel) : null;
    const reinforcementScore = typeof idea.reinforcementScore === 'number' ? `${idea.reinforcementScore}%` : null;
    const suggestedPositionSizing = idea.suggestedPositionSizing ? idea.suggestedPositionSizing : null;
    const executionQuality = idea.executionQuality ? idea.executionQuality : null;
    const enterpriseSectionParts = [];
    if (portfolioRiskLabel)
        enterpriseSectionParts.push(`Portfolio Risk: <b>${portfolioRiskLabel}</b>`);
    if (riskGuardState)
        enterpriseSectionParts.push(`Risk Guard: <b>${riskGuardState}</b>`);
    if (sessionContext)
        enterpriseSectionParts.push(`Session: <b>${esc(sessionContext.session)}</b> — ${esc(sessionContext.description)}`);
    if (marketStressLevel)
        enterpriseSectionParts.push(`Market Stress: <b>${marketStressLevel}</b>`);
    if (reinforcementScore)
        enterpriseSectionParts.push(`Reinforcement: <b>${reinforcementScore}</b>`);
    if (executionQuality)
        enterpriseSectionParts.push(`Execution Slippage: <b>${executionQuality.slippagePct}%</b> | Adjusted Entry: <b>$${executionQuality.entryPrice}</b>`);
    if (suggestedPositionSizing)
        enterpriseSectionParts.push(`Suggested Size: <b>${suggestedPositionSizing.positionSizePct}%</b> | Max: <b>${suggestedPositionSizing.maxPositionPct}%</b>`);
    ...
    const isTest = idea.is_test === true;
    ...
    return (`🚀 <b>${esc(idea.ticker)}${companyLine} ${dir}</b>\n` +
        `📋 ${esc(strategyName)}\n\n` +
        `Entry: <b>${entryLine}</b>\n` +
        ...
        `Risk/Reward: <b>1:${(0, indicators_1.round2)(idea.risk_reward_ratio)}</b>\n` +
