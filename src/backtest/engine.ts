...
const log = createComponentLogger('backtest:engine');
...
export class BacktestEngine {
   ...
   */
  async runBacktest(
    strategyFunction: (candles: Candle[], symbol: string) => Promise<StrategyResult[]>,
    symbol: string,
    candles: Candle[],
    ...
    const filteredCandles = candles.filter(candle =>
      new Date(candle.time) >= config.startDate && new Date(candle.time) <= config.endDate
    );
  ...
  async runStrategyComparison(
    strategies: Array<{
      name: string;
      function: (candles: Candle[], symbol: string) => Promise<StrategyResult[]>;
    }>,
    symbol: string,
    candles: Candle[],
    config: BacktestConfig
  ...
  private executeTrades(
    ...
    for (let i = 0; i < signals.length; i++) {
      ...
      if (openPositions >= config.maxPositions) continue;
      ...
      const positionSize = Math.min(
        config.maxPositionSize * capital / 100,
        capital * 0.1 // Maximum 10% of capital per trade
      );
   ...
   */
  private calculatePerformanceMetrics(trades: TradeRecord[], config: BacktestConfig): BacktestResult {
    if (trades.length === 0) {
      return {
        totalReturn: 0,
        annualizedReturn: 0,
   ...
   */
  private calculateEquityCurve(trades: TradeRecord[], config: BacktestConfig): Array<{ date: Date; equity: number }> {
   ...
   */
  private calculateMaxDrawdown(equityCurve: Array<{ date: Date; equity: number }>): number {
    let maxDrawdown = 0;
   ...
   */
  private calculateMonthlyReturns(trades: TradeRecord[], config: BacktestConfig): Array<{ month: string; return: number }> {
   ...
   */
  validateConfig(config: BacktestConfig): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
