...
const log = createComponentLogger('tracking');
...
function computeProfitPercent(idea: TradeIdea, exitPrice: number): number | null {
  const entry = typeof idea.entry_price === 'number' ? idea.entry_price : null;
  if (entry == null || entry <= 0 || !Number.isFinite(exitPrice)) return null;
...
function computeDurationMinutes(createdAt: string | null | undefined, closedAtIso: string): number | null {
...
function mapExecutionOutcome(event: TrackingEvent, profitPercent: number | null): {
  result: SignalExecutionResult;
  tpHit: 1 | 2 | 3 | null;
  closeReason: string;
} | null {
  if (event === 'tp1_reached') return { result: 'WIN', tpHit: 1, closeReason: 'tp1_reached' };
  if (event === 'tp2_reached') return { result: 'WIN', tpHit: 2, closeReason: 'tp2_reached' };
  if (event === 'time_exit') return { result: 'EXPIRED', tpHit: null, closeReason: 'time_exit' };
...
export async function checkTradeIdea(idea: TradeIdea): Promise<TrackingResult> {
  ...
  const age = hoursSince(idea.created_at);
  if (age > config.risk.maxTradeAgeHours) {
    log.info(`Time exit for ${idea.ticker}: idea is ${round2(age)}h old`);
  ...
  if (idea.status === 'active' || idea.status === 'tp1_reached') {
    ...
    if (
      idea.strategy_slug === 'breakout_volume' &&
      idea.entry_zone_low &&
...
export async function applyTrackingEvent(
  ...
  event: TrackingEvent,
  price: number,
  sendUpdate: (idea: TradeIdea, event: TrackingEvent, price: number) => Promise<void>
): Promise<void> {
  ...
  const statusMap: Partial<Record<TrackingEvent, string>> = {
  ...
  const exitEvents: TrackingEvent[] = [
  ...
  const breakEvenStop =
    event === 'tp1_reached' && idea.entry_price != null
      ? round2(idea.entry_price)
      : undefined;
  ...
  await updateTradeIdeaStatus(idea.id, newStatus, {
    exit_reason: isClose ? event : undefined,
    stop_loss: breakEvenStop,
  });

  await insertTradeUpdate({
...
function buildUpdateMessage(event: TrackingEvent, price: number, idea: TradeIdea): string {
  const msgs: Record<TrackingEvent, string> = {
    ...
    time_exit: `Time-based exit triggered for ${idea.ticker} after ${config.risk.maxTradeAgeHours}h.`,
...
async function persistSignalPerformance(
  idea: TradeIdea,
  event: TrackingEvent,
  exitPrice: number
): Promise<void> {
  try {
    ...
    // Calculate duration
    const durationHours = idea.created_at ? hoursSince(idea.created_at) : 0;
    ...
    await insertSignalPerformance(performanceData);
