...
function parseIntSafe(key: string, fallback: number): number {
  const value = parseInt(optional(key, String(fallback)), 10);
  return Number.isFinite(value) ? value : fallback;
}

function parseFloatSafe(key: string, fallback: number): number {
...
console.info(`[CONFIG] Crypto volume threshold loaded: ${config.crypto.volumeThreshold}`);
console.info(`[CONFIG] Crypto enabled: ${config.crypto.enabled}`);
console.info(`[CONFIG] TEST_MODE: ${config.scanner.testMode}`);
console.info(`[CONFIG] Crypto provider: ${config.crypto.provider}`);
console.info(`[SHORT_CONFIG] stocks.allowShortSelling=${config.scanner.allowShortSelling} (ALLOW_SHORTS/ALLOW_SHORT_SELLING) | crypto.allowShortSelling=${config.crypto.allowShortSelling} (CRYPTO_ALLOW_SHORTS/ALLOW_SHORTS/ALLOW_SHORT_SELLING)`);

export function validateRuntimeEnvironment(): void {
  if (config.security.enforceAdminOnControlRoutes && !config.security.adminToken) {
    throw new Error('Missing required ADMIN_API_TOKEN while ENFORCE_ADMIN_CONTROL_ROUTES=true');
  }

  if (!config.telegram.botToken || config.telegram.botToken.length < 20) {
    throw new Error('TELEGRAM_BOT_TOKEN appears invalid or too short');
  }

  if (!config.supabase.serviceRoleKey || config.supabase.serviceRoleKey.length < 20) {
