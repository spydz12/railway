import { Candle, averageVolume, ema, findResistanceLevels, round2, rsi, safeAtr } from '../../utils/indicators';
import { Strategy, StrategyResult, EMPTY_RESULT } from '../base';
import { createComponentLogger } from '../../utils/logger';
import { config } from '../../config';

const log = createComponentLogger('strategy:crypto_adaptive_momentum');
...
function macdBullishCross(closes: number[]): boolean {
  const ema12 = ema(closes, 12);
  const ema26 = ema(closes, 26);
  if (ema12.length === 0 || ema26.length === 0) return false;
  ...
  if (signal.length < 2 || macdSeries.length < signal.length + 1) return false;
...
function averageAtr(candles: Candle[], period = 14, windows = 6): number {
  if (candles.length < period + windows + 1) return 0;
  const values: number[] = [];
  for (let i = 0; i < windows; i++) {
    const end = candles.length - i;
    const slice = candles.slice(0, end);
    values.push(safeAtr(slice, period));
  }
  return values.reduce((sum, v) => sum + v, 0) / values.length;
}

export class CryptoAdaptiveMomentumStrategy implements Strategy {
  slug = 'crypto_adaptive_momentum';
  ...
  evaluate(candles: Candle[], ticker: string, timeframe: string): StrategyResult {
    ...
    if (relativeVolume <= 1.5) {
      if (debug) log.debug('[NO_SIGNAL] volume not expanded', { ticker, timeframe, relativeVolume });
      return { ...EMPTY_RESULT, strategy: this.slug, symbol: ticker, timeframe };
    }
    ...
    if (!ema21Rising || !ema50Rising) {
      if (debug) log.debug('[NO_SIGNAL] ema trend not rising', { ticker, timeframe, ema21Rising, ema50Rising });
      return { ...EMPTY_RESULT, strategy: this.slug, symbol: ticker, timeframe };
    }

    if (!macdBullishCross(closes)) {
      if (debug) log.debug('[NO_SIGNAL] macd not bullish cross', { ticker, timeframe });
      return { ...EMPTY_RESULT, strategy: this.slug, symbol: ticker, timeframe };
    }

    const vwapVal = vwap(candles);
    if (last.close <= vwapVal) {
      if (debug) log.debug('[NO_SIGNAL] close below vwap', { ticker, timeframe, close: last.close, vwap: vwapVal });
      return { ...EMPTY_RESULT, strategy: this.slug, symbol: ticker, timeframe };
    }
    ...
    if (last.close <= recentHigh * 0.998) {
      if (debug) log.debug('[NO_SIGNAL] breakout not confirmed', { ticker, timeframe, close: last.close, recentHigh });
      return { ...EMPTY_RESULT, strategy: this.slug, symbol: ticker, timeframe };
    }
    ...
    if (atrAvg <= 0 || atrNow < atrAvg * 1.05) {
      if (debug) log.debug('[NO_SIGNAL] atr expansion missing', { ticker, timeframe, atrNow, atrAvg });
      return { ...EMPTY_RESULT, strategy: this.slug, symbol: ticker, timeframe };
    }
    ...
    if (range <= 0 || body / range < 0.45 || last.close <= prev.close) {
      if (debug) log.debug('[NO_SIGNAL] weak candle', { ticker, timeframe, body, range });
      return { ...EMPTY_RESULT, strategy: this.slug, symbol: ticker, timeframe };
    }

    const entry = round2(last.close);
    const stopLoss = round2(entry - atrNow * 1.3);
    const takeProfit1 = round2(entry + atrNow * 2.4);
    const takeProfit2 = round2(entry + atrNow * 3.6);
    const risk = entry - stopLoss;
    if (risk <= 0) return { ...EMPTY_RESULT, strategy: this.slug, symbol: ticker, timeframe };

    let confidence = 70;
    if (relativeVolume > 2.2) confidence += 10;
    if (body / range > 0.6) confidence += 10;
