import { Candle, safeAtr } from '../../utils/indicators';
import { StrategyResult, EMPTY_RESULT } from '../base';
import { createComponentLogger } from '../../utils/logger';

const log = createComponentLogger('strategy:ema_cloud');

export class EMACloudTrendStrategy {
  slug = 'ema_cloud_trend';
  ...
  evaluate(candles: Candle[], ticker: string, timeframe: string): StrategyResult {
    ...
    // Build reasons
    const reasons: string[] = ['ema_cloud_trend'];
    if (bullishTrend) reasons.push('bullish_trend');
    if (bearishTrend) reasons.push('bearish_trend');
    if (pullbackIntoCloud) reasons.push('pullback_rejection');
    if (trendStrength > 0.02) reasons.push('strong_trend');

    // Calculate confidence
    let confidence = 55; // Base confidence
    if (trendStrength > 0.02) confidence += 20;
    if (rejectionCandle) confidence += 15;
