...
const log = createComponentLogger('strategy:mean_reversion');
...
export class MeanReversionStrategy {
  ...
  evaluate(candles: Candle[], ticker: string, timeframe: string): StrategyResult {
    ...
    // Build reasons
    const reasons: string[] = ['mean_reversion'];
    if (oversold) reasons.push('oversold');
    if (overbought) reasons.push('overbought');
    if (divergence) reasons.push('divergence');
    if (rsi2 < 5 || rsi2 > 95) reasons.push('extreme_rsi');

    // Calculate confidence
    let confidence = 50; // Base confidence
    if (divergence) confidence += 20;
    if (rsi2 < 5 || rsi2 > 95) confidence += 15;
    if (trendStrength < 0.01) confidence += 15; // Better in sideways markets
  ...
  private checkDivergence(candles: Candle[], direction: 'bullish' | 'bearish'): boolean {
