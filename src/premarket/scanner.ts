...
const log = createComponentLogger('premarket:scanner');
...
export interface PremarketSignal {
  symbol: string;
  signalType: 'gap_up' | 'gap_down' | 'high_volume' | 'breakout' | 'breakdown';
  strength: number; // 0-100
  confidence: number; // 0-100
  details: {
    gapSize: number;
    volumeMultiplier: number;
    premarketRange: number;
    momentum: number;
  };
...
export class PremarketScanner {
  private readonly MIN_GAP_PERCENT = 2.0; // Minimum gap for consideration
  private readonly MIN_VOLUME_RATIO = 1.5; // Minimum volume ratio vs average
  private readonly HIGH_VOLUME_THRESHOLD = 3.0; // High volume multiplier

  /**
   * Scans premarket data for trading opportunities
   */
  scanPremarketActivity(
    premarketData: PremarketData[],
    averageVolumes: Map<string, number>
  ): PremarketSignal[] {
    const signals: PremarketSignal[] = [];
   ...
   */
  private analyzePremarketData(data: PremarketData, avgVolume: number): PremarketSignal | null {
    ...
    let signalType: PremarketSignal['signalType'];
    let strength = 0;
    let confidence = 0;
    let recommendation: PremarketSignal['recommendation'] = 'WATCH';
  ...
  private calculateGapConfidence(data: PremarketData, volumeRatio: number, premarketRange: number): number {
  ...
  private calculateVolumeConfidence(data: PremarketData, volumeRatio: number, premarketRange: number): number {
  ...
  private calculateRangeConfidence(data: PremarketData, volumeRatio: number, premarketRange: number): number {
  ...
  private calculatePremarketMomentum(data: PremarketData): number {
   ...
   */
  filterQualitySignals(signals: PremarketSignal[]): PremarketSignal[] {
   ...
   */
  getPremarketSummary(signals: PremarketSignal[]): {
    totalSignals: number;
    buySignals: number;
    sellSignals: number;
    watchSignals: number;
