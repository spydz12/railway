...
const log = createComponentLogger('news:sentiment');
...
export interface SentimentAnalysis {
  ...
  urgency: 'BREAKING' | 'TIMELY' | 'ROUTINE';
}
...
export class NewsSentimentEngine {
  ...
  private readonly HIGH_IMPACT_KEYWORDS = new Set([
  ...
  private readonly BREAKING_KEYWORDS = new Set([
   ...
   */
  analyzeNewsSentiment(articles: NewsArticle[]): SentimentAnalysis {
  ...
  analyzeSocialSentiment(
    ...
    const overallSentiment = (
      twitterSentiment.sentiment * 0.3 +
   ...
   */
  getComprehensiveSentiment(
    newsAnalysis: SentimentAnalysis,
    socialSentiment: SocialSentiment
  ): {
    combinedScore: number;
    dominantSource: 'news' | 'social' | 'balanced';
    ...
    const combinedScore = (
      newsAnalysis.overall * newsWeight +
    ...
    let dominantSource: 'news' | 'social' | 'balanced';
    ...
    let tradingImplication: 'BULLISH' | 'BEARISH' | 'NEUTRAL';
    ...
    const confidence = Math.min(100,
      (newsAnalysis.confidence * newsWeight + (socialSentiment.volume > 10 ? 80 : 40) * socialWeight)
    );
  ...
  private extractThemes(articles: NewsArticle[]): string[] {
  ...
  private determineImpact(articles: NewsArticle[], keywords: string[]): 'HIGH' | 'MEDIUM' | 'LOW' {
    ...
    const hasPremiumSource = articles.some(a => premiumSources.some(source =>
    ...
    const recentArticles = articles.filter(a =>
      (now.getTime() - a.publishedAt.getTime()) < (24 * 60 * 60 * 1000) // Within 24 hours
    );
  ...
  private determineUrgency(articles: NewsArticle[]): 'BREAKING' | 'TIMELY' | 'ROUTINE' {
    const allText = articles.map(a => `${a.title} ${a.content}`).join(' ').toLowerCase();

    // Check for breaking keywords
    const hasBreakingKeyword = Array.from(this.BREAKING_KEYWORDS).some(keyword =>
      allText.includes(keyword)
    );

    // Check timing - very recent articles
    const now = new Date();
    const veryRecent = articles.filter(a =>
      (now.getTime() - a.publishedAt.getTime()) < (2 * 60 * 60 * 1000) // Within 2 hours
    );
  ...
  private analyzePlatformSentiment(data: any[]): { sentiment: number; volume: number } {
    if (data.length === 0) {
    ...
    for (const item of data) {
      ...
      if (positiveWords) positive++;
      if (negativeWords) negative++;
    }
  ...
  private calculateSentimentMomentum(sentiments: Array<{ sentiment: number; volume: number }>): number {
