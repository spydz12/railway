...
import { getStrategyRegimePerformanceSummary, StrategyRegimePerformanceSummary } from '../performance/strategyPerformance';
import { ExecutionQualityResult } from '../engine/executionQuality';
import { RiskGuardResult } from '../risk/guard';
import { SessionContext } from '../market/sessionBrain';
import { MarketStressLevel } from '../market/stress';
import { PositionSizingSuggestion } from '../portfolio';
import { PriorTradeOutcome } from '../reinforcement/memory';

const log = createComponentLogger('ai-analyst');
...
export interface SignalCandidate {
  ...
  performanceSummary?: StrategyRegimePerformanceSummary | null;
  recentCandles: any[];
  portfolioRisk?: {
    level: string;
...
export interface AIAnalysisResult {
  ...
  adjustedTakeProfit1: number | null;
  adjustedTakeProfit2: number | null;
}
...
function shouldSkipAIAnalysis(candidate: SignalCandidate): boolean {
  resetDailyCount();
  if (dailyCallCount >= (config.ai.maxCallsPerDay || 50)) {
    log.warn('[AI_ANALYST_SKIPPED] Daily AI limit reached', {
      symbol: candidate.symbol,
      strategy: candidate.strategy,
      callsToday: dailyCallCount,
      maxCalls: config.ai.maxCallsPerDay,
    });
    return true;
  }
  return false;
...
function buildAIPrompt(candidate: SignalCandidate): string {
...
async function callOpenAI(prompt: string): Promise<string | null> {
  ...
  try {
    ...
    log.error('OpenAI API error', { error: (error as Error).message });
    return null;
  }
}
...
function parseAIResponse(response: string): AIAnalysisResult | null {
  try {
    ...
    log.error('Failed to parse AI response', { response, error: (error as Error).message });
    return null;
  }
}

export async function analyzeSignalCandidate(candidate: SignalCandidate): Promise<AIAnalysisResult | null> {
  if (!initializeOpenAI()) {
    return null;
  }
