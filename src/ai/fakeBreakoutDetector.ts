...
const log = createComponentLogger('ai:fakeBreakout');

export interface FakeoutAnalysis {
  isFakeout: boolean;
  confidence: number;
  reason: string;
  details: {
...
export class FakeBreakoutDetector {
  /**
   * Analyzes candles for fake breakout patterns
   * Returns analysis with confidence score for rejecting signals
   */
  analyze(candles: Candle[], breakoutLevel: number, direction: 'up' | 'down'): FakeoutAnalysis {
    if (candles.length < 5) {
      return {
        isFakeout: false,
        confidence: 0,
  ...
  private calculateWickRatio(candle: Candle, direction: 'up' | 'down'): number {
  ...
  private analyzeVolumeDecline(candles: Candle[]): number {
    ...
    const avgPrior = averageVolume(candles.slice(0, -3), 10);
  ...
  private calculateRejectionStrength(candle: Candle, level: number, direction: 'up' | 'down'): number {
    if (direction === 'up') {
  ...
  private analyzeDivergence(candles: Candle[]): number {
    ...
    if (priceHigherHigh && rsiLowerHigh) return 0.8;
    ...
    if (priceLowerLow && rsiHigherLow) return 0.8;

    return 0;
  }

  private analyzeCloseRejection(candle: Candle, level: number, direction: 'up' | 'down'): boolean {
  ...
  private isExhaustionCandle(candle: Candle): boolean {
    const bodySize = Math.abs(candle.close - candle.open);
    const totalRange = candle.high - candle.low;
  ...
  private isLiquidityGrab(candles: Candle[], level: number, direction: 'up' | 'down'): boolean {
  ...
  private calculateRSIArray(closes: number[], period: number): number[] {
